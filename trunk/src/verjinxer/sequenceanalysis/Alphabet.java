/*
 * AlphabetMap.java Created on December 12, 2006, 5:40 AM
 */
...
public class Alphabet {
      ...
      for (String l : lines) {
         if (l.startsWith("##")) {
            ...
            if (icolon >= 0) {
               ...
               if (istring.length() > 0)
                  i = Integer.decode(istring);
               ll = ll.substring(0, icolon);
            }
            int ii = (i < 0) ? i + 256 : i;

            /*
             * at this point, a line like this: "##separators:-1" has been parsed into ll =
             * "separators" i = -1 ii = 255
             */
   ...
   public byte[] applyTo(final String s, final boolean appendSeparator,
         final boolean separateByWildcard) throws InvalidSymbolException {
      int l = s.length();
      byte[] ba = new byte[(appendSeparator ? l + 1 : l)];
      for (int i = 0; i < l; i++)
         ba[i] = code((byte) s.charAt(i));
      if (appendSeparator)
   ...
   public void applyTo(final byte[] s, final boolean setSeparator, final boolean separateByWildcard)
      ...
      for (int i = 0; i < ll; i++)
         s[i] = code(s[i]);
      if (setSeparator)
         s[ll] = (separateByWildcard ? codeWildcard() : codeSeparator());
   }
    ...
    * @throws verjinxer.sequenceanalysis.InvalidSymbolException
    */
   public ByteBuffer applyTo(final String sequence, ByteBuffer buf, final boolean append,
         final byte appendwhat) throws InvalidSymbolException {
      ...
      buf.position(0);
      for (int i = 0; i < ll; i++)
         buf.put(code((byte) sequence.charAt(i)));
      if (append)
         buf.put(appendwhat);
   ...
   public byte[] translateFileToByteArray(final String inname, byte[] translation,
      ...
      if (translation == null || translation.length < ll)
         translation = new byte[ll];
      for (int i = 0; i < length; i++)
         translation[i] = this.code(buf.get());
      if (appendSeparator)
         translation[ll - 1] = this.codeSeparator();
      return translation;
   }
   ...
   public void translateStringToFile(final String s, final String fname, final boolean append,
      ...
      for (int i = 0; i < slen; i++)
         buf.put(this.code((byte) s.charAt(i)));
      if (writeSeparator)
         buf.put(this.codeSeparator());
      fcout.close();
      f.close();
   }
