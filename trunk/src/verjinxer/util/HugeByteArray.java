 ...
 * This particular class provides huge byte arrays.
 * @author Sven Rahmann
 */
public class HugeByteArray {

   /** number of bytes per element */
   public final static int BYTESPERELEMENT = Byte.SIZE/8;
   ...
   /** create a new HugeByteArray 
    * @param size  number of elements in the array, up to (2^63 -1)
    */
   public HugeByteArray(final long size) {
      ...
      for (int i = 0; i < num - 1; i++) {
         arr[i] = new byte[ELEMENTSPERCOMPONENT];
      }
      if (num >= 1) {
         arr[num - 1] = new byte[lastnumel];
      }
   }
    ...
    */
   public HugeByteArray(final HugeByteArray o) {
      length = o.length;
      num = o.num;
      arr = new byte[num][];
      for (int i = 0; i < num; i++) {
    ...
    */
   public final void set(final long i, final byte x) {
    ...
    */
   public final void fill(final byte x) {
      for (int i = 0; i < num; i++) {
         Arrays.fill(arr[i], x);
      }
   }
   ...
   /**
    * Reads a part of a file on disk into a part of this HugeByteArray.
    ...
    * @param fpos     file index (in 'byte' units) at which to start reading
   ...
   public final void read(final String fname, long start, long nItems, final long fpos) throws IOException {
      ...
      final ByteBuffer ib = _bb; // type depends on 'a'
   ...
   /** factory function for constructing a HugeByteArray from a binary file
    * @param fname  the file name
    * @return  a HugeByteArray with the file's contents
    * @throws java.io.IOException 
    */
   public static final HugeByteArray fromFile(final String fname) throws IOException {
     final FileChannel channel = new FileInputStream(fname).getChannel();
     final long flen = channel.size();
     if (flen % BYTESPERELEMENT !=0)
       throw new IOException("File size not compatible with HugeByteArray");
