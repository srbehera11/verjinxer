/*
 * SuffixTrayBuilder.java Created on May 2, 2007, 6:23 PM TODO: include lcp in checking! TODO:
 * include manber myers method! TODO: space-efficient lcp not yet implemented
 */
...
public class SuffixTrayBuilder {
   ...
   public int run(String[] args) {
      ...
      Options opt = new Options(
            "c=check,C=onlycheck,X=notexternal=nox=noexternal,m=method:,l=lcp=lcp4,lcp1,lcp2");
      try {
         args = opt.parse(args);
      } catch (IllegalOptionException ex) {
         g.terminate("suffixtray: " + ex);
      }
      if (args.length == 0) {
         help();
      ...
      for (int p = n - 1; p >= 0; p--) {
         ...
         if (lexfirstpos[chi] == -1) { // seeing character ch for the first time
            ...
            if (alphabet.isSpecial(ch)) { // special character: always inserted first
               ...
               int i = p + 1;
               steps++;
               while (lexprevpos[i] != -1 && s[(i = lexprevpos[i]) - 1] != ch) {
                  steps++;
               }
               i--;
    ...
    * 
    * @param show
    *           write intermediate results to stdout?
    */
         ...
         if (lexfirstpos[chi] == -1) { // seeing character ch for the first time
            ...
            if (alphabet.isSpecial(ch)) { // special character: always inserted first
               ...
               case 1:
                  ...
                  break;
               case 2:
                  insertbetween(pup, lexnextpos[pup], p);
                  if (lexlastpos[chi] == pup)
                     lexlastpos[chi] = p;
                  break;
               case 3:
                  insertaslast(chi, p);
                  break;
               case 4:
                  insertbetween(lexprevpos[pdown], pdown, p);
                  if (lexfirstpos[chi] == pdown)
                     lexfirstpos[chi] = p;
                  break;
               default:
                  g.terminate("suffixtray: internal error");
               } // end switch
            } // end symbol character
         } // end seeing character ch again
         if (show)
            showall_R(String.format("List after step %d: [%d]%n", p, s[p])); // DEBUG
      } // end for p
   }
   ...
   private final void insertbetween(final int p1, final int p2, final int i) {
      // before: ... p1, p2 ...
      // after: ... p1, i, p2 ...
      assert (p1 == -1 || lexnextpos[p1] == p2) : String.format(
            "i=%d (of %d); p1=%d, p2=%d; s[i..i+1]=(%d,%d); lexsucc[p1]=%d %n", i, n, p1, p2, s[i],
            (i < n - 1 ? s[i + 1] : 999), lexnextpos[p1]);
      assert (p2 == -1 || lexprevpos[p2] == p1) : String.format(
            "i=%d (of %d); p1=%d, p2=%d; s[i..i+1]=(%d,%d); lexpred[p2]=%d %n", i, n, p1, p2, s[i],
            (i < n - 1 ? s[i + 1] : 999), lexprevpos[p2]);
      lexprevpos[i] = p1;
      lexnextpos[i] = p2;
      if (p2 != -1)
         lexprevpos[p2] = i;
   ...
   /**
    * lightweight doubly linked list with xor coding. NOTE: uses lexfirstpos[] and lexlastpos[] from
    * enclosing class! does not use lexprevpos[] or lexnextpos[]!
    */
   ...
   /**
    * check correctnes of a suffix array when lexnextpos is available; output warning messages if
    * errors are found.
    * 
   ...
   /**
    * check correctnes of a suffix array constructed with Walk-bothLR using the SPACE SAVING xor
    * technique; output warning messages if errors are found.
    * 
    * @return 0 on success, 1 on sorting error, 2 on count error, 3 on both errors.
    */
   ...
   /**
    * write partial pos array to logfile during a construction where lexnextpos[] is available.
    * 
    * @param header
    *           a header string to print
    */
   private void showall_R(String header) {
      ...
      for (chi = 0; chi < 256; chi++) {
         ...
         // System.out.printf("  char %d: lexfirst=%2d, lexlast=%2d%n", chi-128, lexfirstpos[chi],
         // lexlastpos[chi]);
      }
      for (chi = 0; chi < 256 && lexfirstpos[chi] == -1; chi++) {
      }
      for (p = lexfirstpos[chi]; p != -1; p = lexnextpos[p])
         System.out.printf("  %2d: [%c] %s%n", p, (char) (p > 0 ? s[p - 1] + 48 : '$'),
               ArrayUtils.bytesToString(s, p));
   }
