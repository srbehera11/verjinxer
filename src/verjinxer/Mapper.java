...
public class Mapper {
   ...
   */
  public void help() {
    g.logmsg("Usage:%n  %s map  [options]  <sequences>  <[@]index...> %n", programname);
    g.logmsg("Reports all occurrences of the sequences in the indices%n");
    g.logmsg("in human-readable output format. One or more indices can be given%n");
    g.logmsg("either by filename or in a file of filenames when preceded by @.%n");
    g.logmsg("Writes .mapped and .nonmappable.%n");
    g.logmsg("Options:%n");
    g.logmsg("  -r, --rc                also map reverse complements (DNA only!)%n");
    g.logmsg("  -e, --errorlevel <e>    error level for mapping [0.03]%n");
    g.logmsg("  -c, --clip <num>        clip num characters at each end [0]%n");
    g.logmsg("  -R, --repeat <T>        specify #blocks repeat threshold T [+inf]%n");
    g.logmsg("  -Q, --qcomplexity #|<T> pre-filter sequences by q-vocabulary [0.0]%n");
    g.logmsg("  -s, --select #|<file>   select previously unmapped (#) or specified sequences%n");
    g.logmsg("  -o, --out <filename>    specify output file (use # for stdout)%n");
    g.logmsg("  -m, --method <method>   one of 'qgram' (default), 'suffix', 'full'%n");
    g.logmsg("  --atonce                match against all indices at once (high memory!)%n");
    g.logmsg("QGRAM method options:%n");
    g.logmsg("  -b, --blocksize <b>     blocksize for q-gram mapping [1024]%n");
  ...
  public int run(String[] args) {
    ...
    if (args.length<2) {
      help();
      g.terminate("map: both a sequence file and an index must be specified.");
    }
    tname = args[0];
    dt = g.dir + tname;
    ...
    // Determine options
    int blocksize=1024;
    if (opt.isGiven("e")) errorlevel = Double.parseDouble(opt.get("e"));
    if (errorlevel<0.0 || errorlevel>1.0) g.terminate("map: Illegal error level specified.");
    if (opt.isGiven("c")) clip = Integer.parseInt(opt.get("c"));
    if (clip<0) g.terminate("map: Illegal number of characters to clip specified.");
    if (opt.isGiven("b")) {
      blocksize = Integer.parseInt(opt.get("b"));
      if (blocksize<1) g.terminate("map: Illegal blocksize specified.");
    }
    if (opt.isGiven("R")) repeatthreshold = Integer.parseInt(opt.get("R"));
    if (repeatthreshold<0) g.terminate("map: Illegal repeat threshold specified.");
    if (opt.isGiven("Q")) {
      if (opt.get("Q").startsWith("#")) qgramtabulate=true;
      else qgramthreshold = Double.parseDouble(opt.get("Q"));
    }
    if (qgramthreshold<0 || qgramthreshold>1) g.terminate("map: Illegal q-gram complexity threshold, must be in [0..1].");
    ...
    try {
      for(int i=0; i<inum; i++) {
        String ix = args[i+1];
        if (ix.startsWith("@")) {
          BufferedReader br = new BufferedReader(new FileReader(new File(g.dir+ix.substring(1))));
          String line;
          while((line=br.readLine())!=null) {
            int colon = line.indexOf(':');
            if (colon==-1) indices.add(line);
            else indices.add(line.substring(0,colon));
          }
          br.close();
        } else {
    ...
    for (int i=0; i<inum; i++) {
      ...
      ArrayFile fi = new ArrayFile(fin);
      try {
        fi.openR();
        fi.close();
    ...
    if(selected==0) {
      g.logmsg("map: no sequences selected; nothing to do; stop.%n");
      return 0;
    }
    
    if(qgramthreshold>0 || qgramtabulate) filtersequences();
    ...
    try {
      ...
      } else  allout = new PrintWriter(g.outdir+tname+".allmapped");
    } catch (IOException ex) {
      g.terminate("map: could not create output file; "+ex.toString());
    }
    
    // run chosen method
    if (method==Method.QGRAM && !atonce) mapByQGram(blocksize);
    else if (method==Method.FULL && atonce) mapByAlignmentAtOnce();
    else throw new UnsupportedOperationException("The requested method has not yet been implemented!");
...
// ======================== general index data ==============================
  ...
  int[]       seqbesterror = null;   // keep track of best hits' error
  int[]       seqbesthits  = null;   // number of hits with best error
  int[]       seqallhits   = null;   // number of all hits
...
// ======================== q-gram based methods ==============================
  ...
  /** Proceed index-by-index, then sequence-by-sequence.
  ...
  void mapByQGram(final int blocksize) {
    ...
    g.logmsg("map: allocating %d parallelogram counters, width=%d%n", bhi-blow+1, blocksize);
    ...
    for(int idx=0; idx<inum; idx++) {
      ...
      for (int j=0; j<tm; j++) {
        ...
        if (revcomp && trepeat.get(j)==0) {
          ...
          processQGramBlocks(idx, coder, rcj, 0, jlength, j, -1, clip, bcounter, blocksize, blow);
        }
   ...
   */
  final void processQGramBlocks(final int currenti, final QGramCoder coder,
    ...
    if (q*seqbesterror[currentj] < subtract) subtract = q*seqbesterror[currentj];
    final int bthresh = maxqgrams - subtract;
    // 1. block counters are zero at this point.
    // 2. iterate through all q-grams of txt[0..jlength-1], increase block counters, count high-scoring blocks
    ...
    if (blockhits>=repeatthreshold) { // many block hits, this IS a repeat, probably
      ...
      for (int b=0; b<bcounter.length; b++) {
        if (bcounter[b]>=bthresh) {
          ...
          if (bend>ilength) bend = ilength;
          int endpos = align(txt, jstart+trim, len, itext, bstart, bend, overlap, Dcol); //also have enddelta!
          if (endpos>=0) {
            endpos += trim;
            seqallhits[currentj]++; currenthits++;
            if (enddelta<seqbesterror[currentj]) { seqbesterror[currentj]=enddelta; seqbesthits[currentj]=1; }
            else if (enddelta==seqbesterror[currentj]) { seqbesthits[currentj]++; }
            //g.logmsg("%d%+d: [%s] %d %d%n", currentj, currentdir,   Strings.join("",itext,endpos+1-enddelta-jlength+1, jlength-1+enddelta),  endpos, enddelta);
            //g.logmsg("%d%+d: [%s] %d %d%n", currentj, currentdir,   Strings.join("",itext,endpos+1-jlength+1, jlength-1),  endpos, enddelta);
            //g.logmsg("   > [%s]%n", Strings.join("",txt,jstart,jlength));
            allout.printf("%d %d  %d %d %d   %d %d %d%n",
                currentj, currentdir, currenti, endpos, enddelta,   bthresh, bcounter[b], maxqgrams);
          }
        }
      }
    ...
    if (blockhits>0) 
      g.logmsg(" seq %d%c: %d/%d hits/blocks this time; besterror=%d w. %d best (%d total) hits%n",
        currentj, currentdir>0?'+':'-', currenthits, blockhits, seqbesterror[currentj], seqbesthits[currentj], seqallhits[currentj]);
  }
  ...
  final void incrementBCounters(final int p, final int qcode, final int overlap,
    ...
    for(int r=iqbck[qcode]; r<bckend; r++) {
      ...
      b = d/blocksize - blow;
      ++bcounter[b];
      if ((d%blocksize < overlap) && (b>0)) ++bcounter[b-1];
    }
  }
 ...
 // =============== MAP BY ALIGNMENTS =================================================
  ...
  private final int doTheAlignment(final int currenti, final byte[] itext,
      final byte[] txt, final int jstart, final int jlength, 
      final int currentj, final int currentdir, final int trim) {
          final int len = jlength-2*trim;
          final int tol = (int)java.lang.Math.ceil(jlength*errorlevel);
          //g.logmsg("    tol=%d%n",tol);
          int endpos = align(txt, jstart+trim, len, itext, 0, itext.length, tol, Dcol); //also have enddelta!
          if (endpos>=0) {
            endpos += trim;
            seqallhits[currentj]++;
            if (enddelta<seqbesterror[currentj]) { seqbesterror[currentj]=enddelta; seqbesthits[currentj]=1; }
            else if (enddelta==seqbesterror[currentj]) { seqbesthits[currentj]++; }
            allout.printf("%d %d  %d %d %d%n",
                currentj, currentdir, currenti, endpos, enddelta);
          }
          return endpos;
  }
  ...
  /** align text with parallelogram block #b.
   * Requires blocksize, blow, itext[], asize
   *@param txt  the text array
   *@param start  start position in the text
   *@param len  length of the text (part) to align with a block
   *@param bstart where to start alignming in the index
   *@param bend  where to end aligning in the index (exclusive)
   *@param giventol  absolute error tolerance
   *@param storage  work storage area, must have length >=len
   *@return position where leftmost best match ends in index, -1 if none.
   */
  final int align(final byte[] txt, final int start, final int len, 
      final byte[] index, final int bstart, final int bend, 
    ...
    for(int k=0; k<tol; k++) storage[k]=k+1;
    ...
    for(int c=bstart; c<bend; c++) {
      ...
      if (dmin>tol) {
        while(lei>=0 && storage[lei]>tol) lei--;
        dmin= (lei>=0? storage[lei] : 0);
      } else {
        while(dmin<tol && lei<len-1) { storage[++lei]=++dmin; }
      }
      //lei=newlei;
      assert( (lei==-1 && dmin==0) || dmin==storage[lei]);
      if (lei==len-1 && dmin<bestd) { bestd=dmin; bestpos=c; }
    }
  ...
  /** FULLY align text with parallelogram block #b.
   * Requires blocksize, blow, itext[], asize
   *@param txt  the text array
   *@param start  start position in the text
   *@param len  length of the text (part) to align with a block
  ...
  final int fullalign(final byte[] txt, final int start, final int len, 
    ...
    for(int k=0; k<len; k++) storage[k]=k+1;
    int dup, dul, diag, dmin;
    byte tk;
    for(int c=bstart; c<bend; c++) {
      dup=dul=dmin=0;
      ...
      if (dmin<bestd) { bestd=dmin; bestpos=c; }
    }
    enddelta=bestd; // dirty coding for:: return bestd;
    return bestpos;
  }
  ...
  /** Filter sequences according to contained number of distinct q-grams.
   * Needs asize, tall[], qgram[], ...
   */
  final void filtersequences() {
    ...
    int qq = 4 + (int)(java.lang.Math.ceil(java.lang.Math.log(longestsequence) / java.lang.Math.log(asize-1)));
    if (qgramtabulate) 
      g.logmsg("map: tabulating %d-gram complexity of all reads%n", qq);
    else
      g.logmsg("map: pre-filtering reads with %d-gram complexity < %f%n", qq, qgramthreshold);
    ...
    for (int j=0; j<tm; j++) {
      ...
      seen.clear();
      qcode = -1;
      // iterate through all q-grams of txt[0..jlength-1], increase block counters, count high-scoring blocks
    ...
    if (qgramtabulate) { 
      out.close();
      g.logmsg("map: pre-filtered %d/%d reads due to length in %.2f sec%n", trepeat.cardinality(), tm, timer.tocs());
      g.terminate(0); 
    }
    g.logmsg("map: pre-filtered %d/%d reads due to low %d-gram complexity in %.2f sec%n", trepeat.cardinality(), tm, qq, timer.tocs());
  ...
  final double[] computeEValues(long max, int q, long textlen, int bwidth, double match) {
    ...
    for (int ll=0; ll<=maxlen; ll++) {
      ...
      // the number of q-gram hits H within a parallelogram is geometric, P(H=h) = (1-match)*match^(h-1)
      if (t<=0)  { ev[ll]=1.0; continue; }
