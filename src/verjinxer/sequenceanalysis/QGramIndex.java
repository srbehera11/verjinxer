...
public class QGramIndex {
   ...
   /** q-gram positions as buffer */
   IntBuffer qpos;

   /** q-gram positions as array */
   int[] qposa;
   final boolean external;

   public QGramIndex(final Globals gl, final String qposfile, final String qbckfile,
         boolean external) {
      this.g = gl;
      ...
      if (external)
         qpos = g.mapR(qposfile).asIntBuffer();
      else
         qposa = g.slurpIntArray(qposfile);
   }
    ...
    * already be allocated and have a length of at least bucketSize(qcode).
    * 
    * @param qcode
    *           the q-code corresponding to the desired q-gram
    * @param dest
    *           the destination array into which the positions are copied
   ...
   public void getQGramPositions(int qcode, int[] dest) {
      ...
      if (external) {
         qpos.position(r);
         qpos.get(dest, 0, bucketSize(qcode));
      } else {
         System.arraycopy(qposa, r, dest, 0, bucketSize(qcode));
      }
