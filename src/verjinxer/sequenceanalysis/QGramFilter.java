...
/**
 * This class implements a filter for q-grams based on a BitArray.
 * Each q-gram is represented by a bit. If q-gram 'c' is to be ignored (filtered),
 * bit number c is set to 1, and to 0 otherwise.
 * @author Sven Rahmann
 */
public class QGramFilter {
   ...
   //private final QGramCoder coder;

   public QGramFilter(final int q, final int asize, final int complexity, final int delta) {
      // using a QGramCoder performs range checking on q, asize.
      this(new QGramCoder(q, asize), complexity, delta);
   }

   public QGramFilter(final QGramCoder coder, final int numchar, final int delta) {
      q       = coder.q;
      asize   = coder.asize;
      size    = coder.numberOfQGrams;
      bits    = setFilter(coder, numchar,delta);
   }

   /** create a low-complexity q-gram filter.
    * This is a BitArray with the property that the c-th bit is 1 iff
   ...
   private BitArray setFilter(final QGramCoder coder, final int complexity, final int delta) {
      ...
      if (complexity == 0) return f;           // nothing to do, all-0 filter
    ...
    *   the complexity threshold at index 0,
    *   the delta value at index 1.
    */
   public static int[] parseFilterParameters(final String filterparams) {
