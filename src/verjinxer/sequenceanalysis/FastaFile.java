...
public class FastaFile
{
   ...
   {
     if (in!=null || out!=null) throw new IOException("FastaFile already open");
   ...
   public FastaSequence read() throws IOException, FastaFormatException
   {
      String line;
      FastaSequence seq;
      if (nextHeader!=null) seq = new FastaSequence(nextHeader);
      else
      {
  ...
  /** writes a string to this FASTA file with 60 characters/line.
   * The file must be open for writing; otherwise a runtime error results.
   * The output is preceded by a FASTA header, as given by the header argument
   * @param s       the string to be written
   * @param header  the header for the string
   */
   ...
   /**
    * translates the FASTA file into a AnnotatedArrayFile using the given alphbet map
    * OUT  OF DATE
    * 
    * @param bf the output file, often a <code>new AnnotatedArrayFile(filename)</code>
    * @param amap the alphabet map to use
    * @param append true if an existing file should be appended.
    *   In this case, the existing BiAnnotatedArrayFilehould be passed as first argument.
    * @param amapReverse if not null, alphabet map to use for the reverse sequence
