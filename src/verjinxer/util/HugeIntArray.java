...
/**
 * This class provides huge (&lt; 2 Giga Elements) arrays of basic type int. Since Java limits
 ...
 * arrays with more than 2^31 ~ 2 billion elements. This particular class provides huge int
 * arrays.
 * 
 * @author Sven Rahmann
 */
public class HugeIntArray {
   ...
   /**
    * Creates a new HugeIntArray as an exact copy of an existing one
    * 
    * @param other
    *           the existing array
    */
   public HugeIntArray(final HugeIntArray other) {
      length = other.length;
      bins = other.bins;
      arrays = new int[bins][];
   ...
   /**
    * Creates a new HugeIntArray and wraps a copy of the given primitive int array.
    * 
    * @param other
    *           an existing primitive array
    */
   public HugeIntArray(final int[] other) {
      length = other.length;
      bins = (int) ((length - 1) >> BITS) + 1; // last bin is potentially empty (array of length 0)
    ...
    */
   public long binarySearch(final int key, final long fromIndex, final long toIndex) {
      long low = fromIndex;
      long high = toIndex - 1;
   ...
   /**
    * Reads a part of a file on disk into a part of this HugeIntArray. If the size of this array
    ...
    * @param fpos
    *           file index (in 'int' units) at which to start reading
